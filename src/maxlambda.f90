! --------------------------------------------------------------------
! maxlambda.f90: compute maxlam such that for a given y, X, and gamma
!
...
!
! --------------------------------------------------------------------
SUBROUTINE maxlambda(nvars, nobs, x, y, pf, maxlam)

    IMPLICIT NONE
    ! -------- INPUT VARIABLES -------- !
    INTEGER :: nvars, nobs
    DOUBLE PRECISION :: x(nobs,nvars), y(nobs), pf(nvars), maxlam
    ! -------- LOCAL DECLARATIONS -------- !
    INTEGER :: k, c, nzvars
    DOUBLE PRECISION :: xy(nvars), r(nobs)
    ...
    !--- ADD SMALL NUMBER TO ENSURE b = 0 @ maxlam (DUE TO NUMERICAL IMPRECISION)
    maxlam = maxlam + 1E-5
